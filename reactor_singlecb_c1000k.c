...
int ws_tranmission(struct ntyevent *ev) {
	...
	if (hdr->pl_len < 126) {
		...
		memcpy(ev->wbuffer,payload,BUFFER_LENGTH);
...
int _ws_response(struct ntyevent *ev) {
...
int _ws_tranmission(struct ntyevent *ev) {
...
int ws_response(struct ntyevent *ev) {
	...
	if (ev->ws_status == WS_HANDSHARK) {
		_ws_response(ev);
		ev->ws_status = WS_TRANMISSION;
	} else if (ev->ws_status == WS_TRANMISSION) {
		_ws_tranmission(ev);
	}
